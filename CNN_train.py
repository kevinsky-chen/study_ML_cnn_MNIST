...
# 資料前處理
# training data
training_input = CNN.preProcessor()
...
cnn = CNN.model()
cnn.layers()
cnn.training(training_input_image, training_input_label)

# 建立好的模型進行預測
prediction = cnn.testing(testing_input_image, testing_input_label)
CNN.preProcessor.show_images_labels_predictions(test_feature, test_label, prediction, 0, 15)   # show出前15個testing結果與label的比對狀況

# Save model& weights
cnn.save_all("Mnist_CNN_model.h5")
print("Mnist_CNN_model.h5 模型已儲存")
cnn.save_weights("Mnist_CNN_weights.weight")
print("Mnist_CNN_weights.weight 權重已儲存")
del cnn
